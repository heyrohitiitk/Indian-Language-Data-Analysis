...
top3to2=np.argsort(ratio_india["3-to-2-ratio"].values)[::-1][:3]
low3to2=np.argsort(ratio_india["3-to-2-ratio"].values)[:3]

df1=ratio_india.iloc[top3to2,[0,1]]
df2=ratio_india.iloc[low3to2,[0,1]]

three_to_two_ratio=pd.concat((df1,df2))
three_to_two_ratio.reset_index(drop=True,inplace=True)

three_to_two_ratio.to_csv("Question-4/3-to-2-ratio.csv",index=False)
